...
PREPARE_LOGGING(msg_producer_i)

msg_producer_i::msg_producer_i(const char *uuid, const char *label) :
    msg_producer_base(uuid, label)
{
    // Avoid placing constructor code here. Instead, use the "constructor" function.

}

msg_producer_i::~msg_producer_i()
{
}

void msg_producer_i::constructor()
{
    /***********************************************************************************
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void msg_producer_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &msg_producer_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &msg_producer_i::<callback method>)
        in the constructor.
        ...
        //Add to msg_producer.cpp
        msg_producer_i::msg_producer_i(const char *uuid, const char *label) :
            msg_producer_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &msg_producer_i::scaleChanged);
            addPropertyListener(status, this, &msg_producer_i::statusChanged);
        }

        void msg_producer_i::scaleChanged(float oldValue, float newValue)
        {
            RH_DEBUG(this->_baseLog, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void msg_producer_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            RH_DEBUG(this->_baseLog, "status changed");
        }
            
        //Add to msg_producer.h
...
************************************************************************************************/
int msg_producer_i::serviceFunction()
{
